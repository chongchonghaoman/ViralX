...
| 🤖 **AI 深度拆解** | 9段叙事结构拆解 + 4大转化钩子识别 + 可执行分镜脚本 |
...
5. 在弹窗中查看完整的分析报告（含9段叙事结构拆解 + 钩子推荐）
...
> TikTok 推荐算法筛选视频的核心指标：完播率、互动率、转化率。爆款不是创意出来的，是被算法筛出来的。脚本满足算法偏好的结构，才能获得流量。

| 段 | 名称 | 功能 | 用户心理 | 适用时长 |
|---|------|------|---------|--------|
| 1 | **Hook** | 前 1-2 秒制造停留 | 「等等，什么？」 | 0-2s |
| 2 | **Pain** | 戳中痛点 | 「对，我也有这个问题」 | 2-5s |
| 3 | **Fear** | 放大不解决的后果 | 「不处理真不行」 | 5-8s |
| 4 | **Solution** | 亮出产品 | 「原来有这个东西」 | 8-12s |
| 5 | **Demo** | 展示使用过程 | 「看起来确实好用」 | 12-40s |
| 6 | **Trust** | 提供信任证据 | 「不是在骗我」 | 40-50s |
| 7 | **Price** | 价格锚定 | 「这个价格值了」 | 50-55s |
| 8 | **CTA** | 引导点购物车 | 「现在就买」 | 55-58s |
| 9 | **Closure** | 情绪收尾 | 「没白花时间」 | 58-60s |

**规则**：9 段不一定全用（15 秒视频只用 Hook + Demo + CTA 即可）；**砍段可以，顺序不能变**（Pain 在 Hook 前用户还没停下来，Price 在 Trust 前用户还不信你）。

### 4 大转化钩子

出现频率超过 70%，AI 自动识别并推荐替换方案：

| 钩子 | 写法 | 原理 | 数据 |
|------|------|------|------|
| **复购声明** | 「这是我第三次买了」 | 行为比语言有说服力 | 加购率 +2.7x |
| **口语自我纠正** | 「特别——不对，不是特别好」 | 制造「不是念稿」的真实感 | AI视频必备 |
| **价格悬念** | 「专业 $200，这里 $15」 | 高价参照锚定 | ROAS +1.8x |
| **身份标签** | 「As a busy mom」 | 精准筛选，打透 = 算法推更多 | 人群越小越精准 |

### 裂变脚本生成

一条验证过的爆款 → 自动生成 4 种裂变变体：
1. 换身份标签（面向不同人群）
2. 换使用场景
3. 换价格锚定参照物
4. 多语言版本

...
class ConnectorLauncherTests(unittest.TestCase):
    def test_server_binds_before_pairing_secret_is_issued(self):
        ...
        with (
            patch.object(connector, "create_connector_app", return_value=(app, broker)),
            patch.object(connector, "ExclusiveThreadedWSGIServer", side_effect=build_server),
            patch.object(connector, "request_shutdown_from_running_connector", return_value=False),
        ...
        self.assertIs(app.config["VIRALX_SHUTDOWN_CALLBACK"], server.shutdown)
    ...
    def test_legacy_connector_reopens_pairing_without_starting_another_server(self):
    ...
    def test_modern_connector_is_replaced_before_new_server_binds(self):
        events = []
        app = Mock()
        app.config = {}
        broker = Mock()
        ...
        with (
            patch.object(connector, "request_shutdown_from_running_connector", return_value=True),
            patch.object(connector, "wait_for_connector_exit", side_effect=wait_for_exit),
            patch.object(connector, "request_pairing_from_running_connector") as request_pairing,
            patch.object(connector, "create_connector_app", return_value=(app, broker)),
            patch.object(connector, "ExclusiveThreadedWSGIServer", side_effect=build_server),
            patch.object(sys, "argv", ["connector.py", "--no-open"]),
        ):
        ...
        self.assertEqual(events, ["wait", "bind", "secret"])
        request_pairing.assert_not_called()
        server.server_close.assert_called_once_with()

    def test_replacement_stops_if_old_connector_keeps_the_port(self):
        with (
            patch.object(connector, "request_shutdown_from_running_connector", return_value=True),
            patch.object(connector, "wait_for_connector_exit", return_value=False),
            patch.object(connector, "create_connector_app") as create_app,
            patch.object(sys, "argv", ["connector.py", "--no-open"]),
        ):
            with self.assertRaises(SystemExit):
